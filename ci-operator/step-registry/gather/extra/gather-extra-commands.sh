...
if openshift-tests e2e-analysis --help &>/dev/null; then
    ...
    if curl -fsSL --connect-timeout 10 --max-time 60 --retry 3 --retry-all-errors --retry-delay 5 https://github.com/stedolan/jq/releases/download/jq-1.6/jq-linux64 -o "${ARTIFACT_DIR}/jq"; then
      chmod ug+x "${ARTIFACT_DIR}/jq"
    else
      echo "WARNING: failed to download jq, skipping cluster-operator-status-to-JSON junit conversion"
    fi
    if test -f "${SHARED_DIR}/proxy-conf.sh"; then
        # shellcheck disable=SC1090
        source "${SHARED_DIR}/proxy-conf.sh"
    fi
    if [[ -x "${ARTIFACT_DIR}/jq" ]]; then
